...
node(){
    ...
    stage('deploy to env') {
        ...
        sh 'docker run -d --link rabbitmq --name gateway -p 18080:18080 niknestor/gateway:$BUILD_NUMBER'
	sh 'docker ps'
    }
    stage('create bin') {
    ...
    stage('integration tests') {
        def messages = [
            'curl http://localhost:18080/message -X POST -d \'{"messageId":1, "timestamp":1234, "protocolVersion":"1.0.0", "messageData":{"mMX":1234, "mPermGen":1234}}\'',
            'curl http://localhost:18080/message -X POST -d \'{"messageId":2, "timestamp":2234, "protocolVersion":"1.0.1", "messageData":{"mMX":1234, "mPermGen":5678, "mOldGen":22222}}\'',
            'curl http://localhost:18080/message -X POST -d \'{"messageId":3, "timestamp":3234, "protocolVersion":"2.0.0", "payload":{"mMX":1234, "mPermGen":5678, "mOldGen":22222, "mYoungGen":333333}\''
        ]
